...
import os
from leaderj1001.LambdaNetworks_main.config import load_config
from leaderj1001.LambdaNetworks_main.preprocess import load_data
from leaderj1001.LambdaNetworks_main.model import LambdaResNet18, get_n_params
...
def main(args):
    ...
    summary(model, tuple(torch.Size([3, 32, 32])), device='cpu')
...
if __name__ == '__main__':
    ...
    # The CosineAnnealing optimizer has warm restarts with T0 = 10 and Tmulti = 2
    # They use gradient clip
    # Their LambdaLayer uses Conv2d instead of linear mappings. As a result, they do not need to reshape the image to m
    # However, they later have to resize all the matrices before the einsum
    # They use avg pooling to decrease the size of the lambda layer, instead of a 1x1 Conv downsample block. Less params
    # The self.embedding is ku in size in the case of global context, instead of mkn
    # They do the global value matrix of size n instead of m
    # They do a 3d conv for the embeddings. As a result, the embeddings are not of size mkn but mk
    # They apply dropout of 0.3 in the last classification layer.
